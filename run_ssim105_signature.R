...
data.nmf.fo <- RDataFileOut(paste0(geno.type, ".ssim.nmf"),
                            out.path = paste0(opt$parent_dir, "data"))
data.sig.fo <- RDataFileOut(paste0(geno.type, ".ssim.sigs"),
                            out.path = paste0(opt$parent_dir, "data"))
if (opt$jpg) {
  plot.count.fo <- JpgFileOut(paste0(geno.type, ".ssim.counts"),
                              out.path = paste0(opt$parent_dir, "plots"))
  plot.sig.fo <- JpgFileOut(paste0(geno.type, ".ssim.sigs"),
                            out.path = paste0(opt$parent_dir, "plots"))
  plot.contri.fo <- JpgFileOut(paste0(geno.type, ".ssim.contri"),
                               out.path = paste0(opt$parent_dir, "plots"))
  plot.cos.fo <- JpgFileOut(paste0(geno.type, ".ssim.cos"),
                            out.path = paste0(opt$parent_dir, "plots"))
  if (opt$n_signatures < 2) {
    plot.meas.fo <- JpgFileOut(paste0(geno.type, ".ssim.meas"),
                               out.path = paste0(opt$parent_dir, "plots"))
  }
} else {
  plot.count.fo <- PdfFileOut(paste0(geno.type, ".ssim.counts"),
                              out.path = paste0(opt$parent_dir, "plots"))
  plot.sig.fo <- PdfFileOut(paste0(geno.type, ".ssim.sigs"),
                            out.path = paste0(opt$parent_dir, "plots"))
  plot.contri.fo <- PdfFileOut(paste0(geno.type, ".ssim.contri"),
                               out.path = paste0(opt$parent_dir, "plots"))
  plot.cos.fo <- PdfFileOut(paste0(geno.type, ".ssim.cos"),
                            out.path = paste0(opt$parent_dir, "plots"))
  if (opt$n_signatures < 2) {
    plot.meas.fo <- PdfFileOut(paste0(geno.type, ".ssim.meas"),
                               out.path = paste0(opt$parent_dir, "plots"))
  }
}
...
if (!is.matrix(ssim105) || nrow(ssim105) != 105) {
  stop("Invalid motif matrix. Must be a matrix with 105 rows (mutation types)",
       call. = F)
}

if (ncol(ssim105) < 10 && opt$n_signatures < 2) {
...
ssim105.reduced <- RemoveMinorMutationTypes(ssim105, normalise = opt$normalise)

if (opt$n_signatures > min(nrow(ssim105.reduced), ncol(ssim105.reduced)) - 1) {
  stop("Too large N_SIGNATURES specified", call. = F)
}

# cat("Info: Bootstrapping ...\n")
# ssim105.boot <- McBootstrap(ssim105.reduced)

if (opt$normalise) {
  cat("Info: Normalising data ...\n")
  ssim105.reduced <- sweep(ssim105.reduced, 2, colSums(ssim105.reduced), `/`)
}

if (opt$n_signatures < 2) {
  # Number of signatures is not provided
  cat("Info: Estimating 'k' range ...\n")
  max.k <- EstMaxNumOfSignatures(ncol(ssim105.reduced))
  cat("Info: 'k' in [2, ", max.k, "] will be tested\n", sep = "")

  if (max.k > 2) {
    cat("Info: Running NMF ...\n")
    nmf <- nmf(ssim105.reduced, 2:max.k, "brunet", seed = 42,
               nrun = opt$iteration)
    ...
    sigs <- ConstSignatures(nmf, ssim105.reduced, cosine.low = opt$cosine_low,
                            cosine.high = opt$cosine_high)
  } else {
    cat("Info: Estimated number of signatures: 2\n")
    sigs <- identifySignatures(ssim105.reduced, 2, nmfDecomposition, seed = 42)
  }
} else {
  # Number of signatures is provided
  cat("Info: Input number of signatures: ", opt$n_signatures, "\n", sep = "")
  sigs <- identifySignatures(ssim105.reduced, opt$n_signatures,
                             nmfDecomposition, seed = 42)
}
...
PlotMutationTypeCounts(ssim105, count.plotter, geno.type = geno.type,
                       opt$log10_count)

sig.plotter <- SignaturePlotter(plot.sig.fo)
PlotSsim105Signatures(sigs, sig.plotter, geno.type = geno.type)
